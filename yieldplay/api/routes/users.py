...
#     try:
#         return sdk.approve_token(
#             body.token_address, int(body.amount_wei) if body.amount_wei else None
#         )
#     except Exception as exc:
#         raise handle_sdk_error(exc) from exc
...
# @router.get(
#     "/{user_address}/deposits/{game_id}/{round_id}",
#     response_model=UserDepositInfo,
#     summary="Raw user deposit from contract",
# )
# async def get_user_deposit(
#     user_address: str, game_id: str, round_id: int, sdk: SDKDep
# ) -> UserDepositInfo:
#     try:
...
# @router.get(
#     "/{user_address}/balance/{token_address}", response_model=TokenBalanceResponse
# )
# async def get_token_balance(
#     user_address: str, token_address: str, sdk: SDKDep
# ) -> TokenBalanceResponse:
#     try:
...
# @router.get("/{user_address}/allowance/{token_address}", response_model=dict[str, str])
# async def get_token_allowance(
#     user_address: str, token_address: str, sdk: SDKDep
# ) -> dict[str, str]:
#     try:
#         return {
#             "allowance_wei": str(sdk.get_token_allowance(token_address, user_address))
#         }
#     except Exception as exc:
#         raise handle_sdk_error(exc) from exc
...
# @router.get(
#     "/{user_address}/rounds/{game_id}/{round_id}/summary",
...
# @router.get(
#     "/{user_address}/eligibility/deposit/{game_id}/{round_id}",
...
# ) -> DepositEligibility:
#     try:
#         return await svc.check_deposit_eligibility(
#             user_address, game_id, round_id, amount_wei
#         )
#     except Exception as exc:
#         raise handle_sdk_error(exc) from exc
...
# @router.get(
#     "/{user_address}/eligibility/claim/{game_id}/{round_id}",
...
# ) -> UserPortfolio:
#     try:
#         return await svc.get_user_portfolio(
...
# ── Writes → moved to /api/v1/tx/build/* (non-custodial) ─────────────────
#
# /users/deposit, /users/claim, /users/approve are REMOVED.
# Use the non-custodial flow instead:
#
#   1. POST /api/v1/tx/build/deposit   → unsigned tx
#   2. User signs with MetaMask / WalletConnect
#   3. POST /api/v1/tx/broadcast       → tx_hash
#
# This ensures the server NEVER holds user private keys.
